...
void AT24_TWI_Write(uint08 ByteAddr,uint08 Str)
{ TWI_Start();                                 //开启发送     
  TWI_Write(AT24C_Add_W);               //发送从机地址
  if(TWI_ACK()==SLAW)                        //应答
   TWI_Write(ByteAddr);                       //发送字节地址
...
//读取模式为随机读
uint08 AT24_TWI_Read(uint08 ByteAddr)
{ uint08 u08temp;
  TWI_Start();                                  //发送起始信号                     
  TWI_Write(AT24C_Add_W);                     //写入地址及页选择位
  if(TWI_ACK()==SLAW)                         //地址发送应答
    TWI_Write(ByteAddr);                       //写入字节高地址
...
//------------------------------------------------------------------
//写入数据函数，当前最多写入256个数据
void Write_AT24(uint08 ByteAdd,uint08 ch[])
{ uint16 u16i;
  uint16 u16n=0;
  u16n=strlen(ch);                               //字符长度
  for(u16i=0;u16i<u16n;u16i++,ByteAdd++)
  {  
     AT24_TWI_Write(ByteAdd,ch[u16i]);  //向AT24C04写入数据
     delay(5);                                  //延时5ms
...
void Read_At24(uint08 *pchar,uint08 ByteAdd,uint08 u8len)
{uint08 u08i;

for(u08i=0;u08i<u8len;u08i++,ByteAdd++)                                    
{
   *(pchar+u08i)=AT24_TWI_Read(ByteAdd);  //读一个字节，写入缓冲区
...
void setup() {
  ...
  sei();                       //中断使能
//  #ifdef WRITE
   //Write_AT24(0,Send_Buff);  Serial.println("Write OK.");
}
void readiic(){   
//#else
